...
const Finance = () => {
  ...
  useEffect(() => {
    ...
    const lookup1 = async () => {
      ...
      console.log(proc)
      console.log(saleA)
      const operMin = getOperationLog(prodA)

      const procLog = getProcurementLog(proc)

      const monthC = getMonthlyCosts(operMin, procLog)

      const monthCP = getMonthlyCostsPlan(prodP)

      const monthSP = getMonthlySalesPlan(salesP)

      const salesLog = getSalesLog(saleA)
  ...
  const getOperationLog = (prodActual) => {
    ...
    for (i = 0; i < 12; i++) {
      let monthSum = 0
      if (prodActual[displayYear][monthNames[i]] === undefined) continue
      for (const plantName in prodActual[displayYear][monthNames[i]]) {
        let plantSum = 0
        const plant = prodActual[displayYear][monthNames[i]][plantName]
        for (const machineName in plant) {
          const machine = plant[machineName]
  ...
  const getMonthlyCosts = (operationalMinutes, monthlyProcurements) => {
    ...
    for (let i = 0; i < 12; i++) {
      const c =
        operationalMinutes[i] * operationCostPerMinute + monthlyProcurements[i]
  ...
  const getMonthlyCostsPlan = (prodPlans) => {
    ...
    for (i = 0; i < 12; i++) {
      let monthSum = 0
      if (prodPlans[displayYear][monthNames[i]] === undefined) continue
      for (const plantName in prodPlans[displayYear][monthNames[i]]) {
        let plantSum = 0
        const plant = prodPlans[displayYear][monthNames[i]][plantName]
        for (const itemName in plant) {
          plantSum += plant[itemName]
        }
        monthSum += plantSum
      }
      updatedMonthlyCostsPlan[i] =
        monthSum * 45 * operationCostPerMinute + monthSum * 100
    }
  ...
  const getMonthlySalesPlan = (salesPlans) => {
    ...
    for (i = 0; i < 12; i++) {
      if (salesPlans[displayYear][monthNames[i]] === undefined) continue
  ...
  const getMonthlyProfitsPlan = (monthlySalesPlans, monthlyCostsPlan) => {
    const updatedMonthlyProfitsPlan = []
    for (let i = 0; i < 12; i++) {
      const p = monthlySalesPlans[i] - monthlyCostsPlan[i]
  ...
  const getMonthlyProfits = (monthlySales, monthlyCosts) => {
    const updatedMonthlyProfits = []
    for (let i = 0; i < 12; i++) {
      const p = monthlySales[i] - monthlyCosts[i]
              ...
              <Row className="mb-2 ml-2 mr-2 mt-2">
                ...
                <Col className="">
                  <Card className="shadow">
                    <CardHeader className="bg-transparent text-center">
                      <h1 className=" mb-0">
                        Monthly Breakdown for {monthNames[displayMonth]}
                      </h1>
                      <Form>
                        <FormGroup className="mb-3">
                          <label>
                        ...
                        <Bar
                          data={{
                            ...
                            datasets: [
                              {
                                data: [
                                  monthlySales[displayMonth],
                                  monthlyCosts[displayMonth],
                                  monthlyProfits[displayMonth],
                                ],
                                backgroundColor: [
                        ...
                        <Doughnut
                          data={{
                            ...
                            datasets: [
                              {
                                data: [
                                  operationalMinutes[displayMonth] * 0.05,
                                  monthlyProcurements[displayMonth],
                                ],
                                backgroundColor: [
                                  colors.theme.info,
